%% function to translate multipole-to-multipole coefficients from bottom to top (M2M)
function M = M2M(tree,k,pp)
...
for cc = 1:2  % loop over each child
    ...
    for i = 0:pp  % row index = power
        for j = 0:i
            coeff = nchoosek(i, j);
            R(i+1,j+1) = coeff*(h^(i-j));  % h^{i-j} * binomial coefficient
        end
    end
    M = [M, R*x];
end
