%% function to compute the Multipole_to_Local coefficient expansion for the derivative part

function tree = M2L_L2L_der(tree,k,pp)
% der_blk
...
bin_coeff = zeros(pp+1,pp+1); % matrix to hold binomial expansion coefficients
  for b = 1:pp+1
      bin_coeff(:,b) = ((-1)^(b+1))*negbin_coeffs(b+1,pp+1); % (x-y)-n coefficients
  end
...
for i1 =   1:numel(list1)
 ...
 kk = list1(i1);    
    ...
    for b = 1:pp+1
        d_matrix(:,b) = d.^(b+1:b+1+pp);
    end
    
    M2L_matrix = bin_coeff.*d_matrix;  % multipole to local translation matrix
    
    farfield_coeff = tree(kk).x2_coeff;
    farfield_sum   = sum(farfield_coeff,2);

    tree(k2).der_blk = tree(k2).der_blk + M2L_matrix*farfield_sum;
end
...
%         k4 = i2+1;
%         tree = L2L_der(tree,path_1(k4),pp); % L2L translation till target node
%     end
% der_blk = tree(k).der_blk;
